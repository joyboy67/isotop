...
def read_config(config):
    ...
    for s in config.sections():
        cf[s] = [ [i.strip() for i in infos.split(',') ] 
                        ...
                        if option != "closeafterrun" and option != "theme" ]

    return cf

def closeafter(config_file):
    """return True if closeafter option is set"""
    config = get_config(config_file)
    if config.has_option("DEFAULT","closeafterrun"):
        return config.getboolean("DEFAULT","closeafterrun")
